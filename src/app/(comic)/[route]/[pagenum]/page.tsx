"use server"
/**----------------------------------- */
// TYPES
import { Metadata } from "next"
// LIBRARIES
import { notFound } from "next/navigation"
// DATA
import { verifySession } from "@/data/session"
import { getSettings } from "@/lib/directus/get-settings"
import { getComic, getComicPage, getComicVariables } from "@/lib/directus/get-comics"
// UI
import { comicPageMetadata, notFoundMetadata } from "../../_ui/metadata"
import ComicPageUI from "../../_ui/page/page-comic"
import { getUserVarsCookie } from "../../_actions/variables"
import { CommentsSection } from "../../_ui/page/page-comic-comments"
import { getComments } from "@/lib/directus/get-comments"
import { readItems } from "@directus/sdk"
import { adminClient } from "@/lib/directus/clients"

/**-----------------------------------
 * COMIC SINGLE SUBPAGE
 * ---
 * - Checks if the dynamic route params are valid based on the layout mode selected
 * - Renders UI or throws a 404 based on the layout mode selected
 * - Generates comic single page metadata or based on the layout mode selected
 * 
 * ---
	// LAYOUT MODE 1: SINGLE COMIC SITE
 * - Only available when there is only one comic
 * - Display comic at the root 
 * - Subpages would be accessible at i.e. `dungeoncomic.com/1`
 * - THIS ROUTE IS 404
 * 
	// LAYOUT MODE 2: MULTI-COMIC SITE
 * - All comics live in their subfolder `dungeoncomic.com/comicslug`
 * - Subpages would be accessible at i.e. `dungeoncomic.com/comicslug/1`
 * 
 */
export default async function ComicPagenumPage({
	params
}: {
	params: Promise<{ route: string, pagenum: number }>
}) {
	// GET THE ROUTE PARAMS
	const { route, pagenum } = await params
	// CHECK IF `single_comic_site` HAS BEEN SET
	const settings = await getSettings()
	const singleComicSite = settings.single_comic_site

	/**----------------------------------- */
	// LAYOUT MODE 1: SINGLE COMIC SITE
	// - Throw 404
	if (singleComicSite) notFound()

	/**----------------------------------- */
	// LAYOUT MODE 2: MULTI-COMIC SITE
	// Throw 404 if the pagenum param is not a number
	if (isNaN(pagenum)) notFound()

	// Get the user session
	const session = await verifySession()
	// Get the User Variable Cookie
	const comic = await getComic({ slug: route })
	const userVariables = await getUserVarsCookie({ comic: comic })
	// Get the comic page IF it is published
	const variables = await getComicVariables(route)
	const comicPage = await getComicPage(route, pagenum)
	// Get comments
	if (!comicPage || comicPage && comicPage.status !== "published") {
		notFound()
	} else {

		const comments = await getComments(comicPage.id as number)
		return <>
			<ComicPageUI
				page={comicPage}
				variables={variables}
				userVariables={userVariables}
				session={session}
			/>

			<CommentsSection
				page={comicPage}
				comments={comments}
				session={session}
				variables={variables}
				userVariables={userVariables}
			/>

		</>
	}
}

/**-----------------------------------
 * Generate Metadata
 * ---
 **/

export async function generateMetadata({
	params
}: {
	params: Promise<{ route: string, pagenum: number }>
}): Promise<Metadata | undefined> {
	// GET THE ROUTE PARAMS
	const { route, pagenum } = await params
	// CHECK IF PAGENUM IS A NUMBER
	let comicPage

	/**----------------------------------- */
	// RETURN PAGE METADATA IF IT EXISTS
	if (!isNaN(pagenum)) {
		comicPage = await getComicPage(route, pagenum)
		if (comicPage) return await comicPageMetadata(route, pagenum)
	}

	/**----------------------------------- */
	// THROW 404 
	return await notFoundMetadata()

}

/**-----------------------------------
 * Generate Static Params
 * ---
 **/
/*
export async function generateStaticParams({
	params: { route },
}: {
	params: { route: string }
}) {
	const settings = await getSettings()

	// Generate params for this subroute ONLY if there is no frontpage comic
	if (!settings.frontpage_comic) {
		const comicPages = await adminClient.request(
			readItems("pages", {
				limit: -1,
				filter: {
					comic: {
						slug: {
							_eq: route
						}
					}
				},
				fields: [
					{ comic: ["slug"] },
					"comic_pagenum"
				]
			})
		)
		// console.log(comicPages.map((page) => ({
		// 	route: page.comic.slug,
		// 	pagenum: page.comic_pagenum.toString()
		// })))
		return comicPages.map((page) => ({
			route: page.comic.slug,
			pagenum: page.comic_pagenum.toString()
		}))
	}
	return []
}*/